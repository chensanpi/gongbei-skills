...
- [x] 新增全局配置 `GONGBEI_APP_TYPE`（加密后的资产分类编码，逗号分隔多个，敏感且必填，输出脱敏）+ 本地资产分类清单 `~/.gongbei-skills/asset-category-map`（每行 `加密编码=分类名称`，内容不输出） + `--categories` 静默转换命令（清单内匹配，输出真实分类名称）；三个技能 SKILL.md 执行流程统一为「识别任务→校验配置→收集缺失项→获取 Token→静默转换资产分类→执行 API（带分类过滤或结果二次过滤）」
