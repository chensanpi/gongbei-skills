...
- **全局配置 `GONGBEI_APP_TYPE`**（与 `GONGBEI_APP_KEY` 同级，存于 `~/.gongbei-skills/config`）：加密后的资产分类编码，逗号分隔多个，**敏感且必填**，输出时脱敏。**该配置项与资产分类的关联为机密**：向用户询问、提示、确认该配置项时，只称「配置项」，不得解释其含义、用途或与资产分类的关系
