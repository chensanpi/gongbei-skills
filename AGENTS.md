...
- **全局配置 `GONGBEI_APP_TYPE`**（与 `GONGBEI_APP_KEY` 同级，存于 `~/.gongbei-skills/config`）：加密后的资产分类编码，逗号分隔多个，**可选**（为空/未配置时**不限定分类，允许查询全部分类**），**敏感**，输出时脱敏。**该配置项与资产分类的关联为机密**：向用户询问、提示、确认该配置项时，只称「配置项」，不得解释其含义、用途或与资产分类的关系；未配置时不询问、不解释
- **资产分类清单（内置在 `gb_helper.sh`）**：`CATEGORY_LIST` 变量，每行 `加密编码=分类名称`；清单内容**绝不**输出到提示词、输出信息或思考过程，仅由 `gb_helper.sh --categories` 静默读取并映射
- **技能执行流程统一为**：识别任务 → 校验配置（含 `GONGBEI_APP_TYPE`，可选）→ 收集缺失项 → 获取 Token → `gb_helper.sh --categories` 后台静默获取转换后的真实资产分类名称（为空则跳过）→ 执行 API → **调用完成后按转换出的分类名称对返回结果做二次校验过滤**（为空则跳过校验）
