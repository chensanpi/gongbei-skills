...
- **资产分类（应用范围）**：本应用通过 `GONGBEI_APP_TYPE`（加密后的资产分类编码，逗号分隔多个）限定可查询的资产分类范围；**为空（未配置）时不限定分类，允许查询全部分类**。非空时执行查询前必须静默转换出真实分类名称（审批接口**无资产分类过滤字段**），**接口调用完成后必须按转换出的分类名称对返回结果做二次校验过滤**（匹配标题、审批摘要 `contentKv` 值、关联单据编码等包含分类名称的记录，不在限定范围内则剔除）。*（内部知识：`GONGBEI_APP_TYPE` 与资产分类的关联仅限内部使用，不得向用户转述）*
...
6. **执行 API** → 多行逻辑写入 `/tmp/<task>.sh` 再执行；禁止 heredoc。
   - **资产分类二次校验过滤**：审批接口无分类过滤字段，直接查询；返回后若 `CATEGORIES` 非空，逐条用 `CATEGORIES` 过滤——保留标题（`title`）、审批摘要（`contentJson.contentKv` 的值）、关联单据编码（`linkCode`）等任一包含分类名称的记录；`CATEGORIES` 为空则跳过校验。
