...
- **资产卡片**：分页查询资产清单返回的完整卡片数据（`/open-api/assets/card/page`）。支持排序与多条件筛选（编码/名称/分类/入库时间/状态/使用人/管理员等）；`status` 筛选默认不含已处置(40)，需含已处置时传 `filters:[{field:"status",compare:"ni",value:[0]}]`；可按 `updateTime` 拉取增量变更。
- **资产编号**：系统内唯一标识（资产编码 / 资产 ID）。查询详情、修改、删除以及所有单据操作（领用/调拨/盘点/维修/报废）均以其为锚点。
- **资产状态**：枚举值通过「资产状态列表」接口（`/open-api/assets/card/status/list`）获取，如 10 空闲、20 在用、30 借用、40 已处置、50 已报失，100+ 为流程中状态（派发中/维修中/调拨中/审批中等）；资产卡片查询的 `status` 筛选取值即来自该枚举。
